...
export default function setSlackGateway(app) {
    registerRoutes(app);

    logger.info(`Slack command server set. Listening on ${config.ports.uag}`);
}

function registerRoutes(app) {
    app.post('/slack/command', collectRawBody, authenticateSlackCommand, bodyParser.urlencoded({extended: true}), routeCommand);
}
